...
>
> 특히 반복 이용을 기반으로 운영되는 플랫폼 특성상, 초반 이용 경험은 장기 이용 여부를 결정짓는 중요한 요인으로 작용할 수 있습니다. 따라서 신규 이용자의 초기 행동 패턴을 분석하고, 이탈 가능성을 사전에 식별할 수 있는 데이터 기반 예측 체계가 필요합니다.
...
>
> 이러한 고빈도 행동 로그 환경은 시계열 기반 행동 변화 분석 및 조기 이탈 탐지 모델을 적용하기에 적합한 도메인 구조를 제공합니다.
...
> 특히, 온라인 환경에서는 플랫폼 간 이동이 용이하기 때문에 초기 이탈은 단순 활동 감소를 넘어 합법 플랫폼의 이용자 기반 약화로 이어질 수 있다.
> 따라서 신규 유입 이용자의 초기 활동 패턴을 분석하고, 이탈 가능성을 사전에 예측하여 선제적으로 관리하는 체계가 필요하다.
...
<section id="conclusion">
  ...
  <h3>7.1 분석 결과 요약</h3>
  <p>
    본 분석은 <strong>Bwin</strong>의 고객 데이터를 바탕으로 이탈 유저의 행동 패턴을 규명하고, 리텐션 강화를 위한 핵심 동인을 파악하는 데 집중하였습니다. 데이터 분석 결과, 고객의 이탈은 단순한 금전적 손실(ROI)보다는 <strong>초기 활동의 연속성</strong>과 <strong>서비스 경험의 다양성 부재</strong>에서 기인함을 확인하였습니다.
  </p>
  ...
  <ul>
    <li>
      <strong>이탈 유저의 전형적 특성: 일회성 베팅 성향</strong>
      <ul>
        <li>이탈 유저의 베팅 규모와 건수는 잔존 유저 대비 현저히 낮으며, 누적 활동 중앙값이 10일 미만인 점을 고려할 때 이들은 서비스 안착 전 이탈하는 탐색적 성향이 강한 것으로 나타납니다.</li>
        <li>수익률(ROI) 및 승률 지표는 잔존 유저와 유의미한 차이를 보이지 않았으며(상관계수 -0.01), 이는 고객이 경제적 손실 때문이 아니라 서비스 지속에 대한 흥미나 동기부여 부족으로 인해 이탈함을 시사합니다.</li>
      </ul>
    </li>
    <br />
    <li>
      <strong>초기 30일: 리텐션 확보의 골든타임</strong>
      <ul>
        <li>누적 활동 일수와 이탈 간의 강한 음의 상관관계(-0.35)는 활동 지속성이 리텐션의 핵심임을 증명합니다.</li>
        <li>누적 활동 7일 이내에 유저의 22.4%, 30일 시점에는 57.1%가 이탈하는 급격한 하강 곡선이 관찰됩니다.</li>
        <li>이는 가입 초기 1주일 이내에 유저에게 강력한 서비스 경험과 접속 동기를 제공하여 '누적 활동 30일'의 임계점을 돌파하게 만드는 것이 비즈니스의 최우선 과제임을 의미합니다.</li>
      </ul>
    </li>
    ...
    <li>
      ...
      <ul>
        <li>베팅 유형별 분석 결과, Fixed Only 유형(이탈률 43.0%)보다 Fixed와 Live를 병행하는 Mixed 유형의 이탈률(19.5%)이 2.2배 이상 낮게 나타났습니다.</li>
        <li>이는 다양한 서비스를 복합적으로 이용하는 유저일수록 플랫폼에 대한 높은 관여도와 고착도를 형성하여 이탈 가능성이 현저히 낮아짐을 의미합니다.</li>
      </ul>
    </li>
  </ul>
  ...
  <blockquote>
    <ul>
      <li><strong>온보딩 전략 수정:</strong> 신규 유저가 초기 7일 이내에 다양한 베팅 유형을 경험할 수 있도록 유도하는 미션이나 보상 설계를 통해 일회성 유저를 혼합형(Mixed) 유저로 전환시켜야 합니다.</li>
      <li><strong>예측 기반 선제 대응:</strong> 수익률 지표보다는 활동 빈도 및 베팅 유형 변화를 실시간으로 모니터링하여, 활동 주기가 길어지거나 단일 유형에만 머무르는 위험군을 조기에 식별하고 맞춤형 리텐션 마케팅을 전개할 것을 제안합니다.</li>
    </ul>
  </blockquote>
</section>
