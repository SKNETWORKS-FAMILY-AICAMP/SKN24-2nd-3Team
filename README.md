...
- 이탈 여부는 위 데이터셋의 회사인 스포츠 베팅 회사 Bwin의 휴먼 전환 일자를 기준으로 1년 1개월, 395일을 기준으로 했습니다.
    - 출처 | "https://help.bwin.com/en/general-information/account/inactive"

- 관측기간인 2005 ~ 2006.08.31 기간 내에 활동이 존재하는 회원 중, 마지막 날짜를 기준으로 더이상 활동하지 않는 회원이 약 30% 이상인 점을 파악하였습니다.
...
- 수익률(ROI)은 이탈과 상관관계가 거의 없다는 것을 파악하였습니다.(-0.01)
...
수익률 및 승률은 잔존 유저와 큰 차이가 없는 것으로 나타남을 확인했습니다.
...
- 튜닝 후 성능 변화 거의 없음 → **선형 모델의 표현력 상한에 도달**
- Train F1 0.66 / Val F1 0.65 — 안정적, 과적합 없음
- **핵심 피처**: `total_active_days` 계수 **-9.52** (이탈 억제 단일 최강 신호)
...
> "얼마나 꾸준히 플랫폼에 접속하느냐"가 이탈을 결정하는 단일 최강 신호
...
> "금액(얼마)"보다 "횟수(얼마나 자주)"가 이탈 방지에 더 중요
...
### 1. 운영 투입 모델: XGBoost (threshold=0.5724)

| 지표      | 수치      | 의미                                |
| --------- | --------- | ----------------------------------- |
| Recall    | **0.814** | 이탈 고객 10명 중 8명 탐지          |
| Precision | 0.678     | 이탈 예측 10건 중 6.8건이 실제 이탈 |
| F1-Score  | **0.740** | Precision-Recall 균형 최고          |
| FP (오탐) | 1,006건   | 마케팅 개입 비용으로 충당 가능      |
| FN (미탐) | 484건     | 놓치는 이탈 고객 — 최소화 필요      |

### 2. 이탈 조기 탐지 기준

```
우선순위 경보 조건:
1. total_active_days 최근 7일 미접속 또는 증가 속도 둔화
2. fixed_bet_cnt / live_bet_cnt 주간 감소율 > 30%
3. hit_days 연속 0 (3주 이상 미적중)
```
...
| 위험 세그먼트  | 탐지 신호           | 권고 액션                     |
| -------------- | ------------------- | ----------------------------- |
| 활동일 감소    | 접속 간격 증가      | 재참여 푸시 알림, 이벤트 안내 |
| 베팅 빈도 급감 | 주간 bet_cnt ↓30%   | 소액 보너스·프로모션 제공     |
| 연속 미적중    | hit_days 0 (3주)    | 베팅 팁 콘텐츠 제공           |
| 신규 초기 이탈 | days_since_reg < 30 | 온보딩 경험 강화              |

---

* **이탈 유저의 전형적 특성: 일회성 베팅 성향**
    * 이탈 유저의 베팅 규모와 건수는 잔존 유저 대비 현저히 낮으며, 누적 활동 중앙값이 10일 미만인 점을 고려할 때, 일회성 베팅 성향이 강하다는 것을 볼 수 있음
    * 수익률(ROI) 및 승률 지표는 잔존 유저와 유의미한 차이를 보이지 않았으며(상관계수 -0.01), 이는 고객이 경제적 손실 때문이 아니라 서비스 지속에 대한 흥미나 동기부여 부족으로 유추할 수 있음

* **초기 30일의 중요성**
    * 누적 활동 7일 이내에 유저의 22.4%, 30일 시점에는 57.1%가 이탈하는 것을 확인함
    * 이는 활동 초기 1주일 이내에 유저에게 강한 흥미와 접속 동기를 제공하여 **'누적 활동 30일'** 달성이 최우선 과제임을 의미함

* **서비스 다양성**
    * 베팅 유형별 분석 결과, Fixed Only 유형(이탈률 43.0%)보다 Fixed와 Live를 병행하는 **Mixed 유형의 이탈률(19.5%)이 2.2배 이상 낮게** 나타났음
    * 이는 다양한 서비스를 복합적으로 이용하는 유저일수록 이탈 가능성이 현저히 낮아짐을 의미함
